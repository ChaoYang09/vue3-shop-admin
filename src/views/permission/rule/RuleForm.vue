...
<script lang="ts" setup>
  ...
  import { getMenu, createMenu, updateMenu, getMenuTree } from '@/api/menu'
  ...
  const loadMenus = async () => {
    const [_err, data] = await getMenuTree()
    if (!_err) {
  ...
  const loadMenu = async () => {
    ...
    const data = await getMenu(props.ruleId)
